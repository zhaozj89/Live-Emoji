...
class ParticleFilter {
	...
	init ( width, height, numParticles, dt ) {
		...
		for ( let i = 0; i < numParticles; ++i ) {
			let particle = new Particle( this.x, this.y, this.theta, this.dx, this.dy, this.dtheta, this.numParticles );
	...
	correct ( x, y, theta ) {
		...
		let sumWeight = 0;
		for ( let i = 0; i < this.numParticles; ++i ) {
			let distX2 = ( this.particles[ i ].x - x ) * ( this.particles[ i ].x - x );
			let distY2 = ( this.particles[ i ].y - y ) * ( this.particles[ i ].y - y );
